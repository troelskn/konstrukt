 ...
 * one instance of wire_Container.
 */
class wire_Container implements wire_iLocator, wire_iCreator
{
  protected $registry;
  protected $emptyArguments;
  protected $defaultFactory;
  /**
   * @var [] wire_iClassFactory
   */
  ...
  function __construct() {
    $this->registry = new wire_Registry();
    $this->registry->set('wire_Creator', new wire_Creator($this));
    $this->emptyArguments = new wire_UserlandArguments();
    $this->defaultFactory = new wire_DefaultFactory();
  }
  ...
  function get($classname) {
    ...
    if ($this->preventLocatorPropagation && is_a($classname, 'wire_iLocator')) {
      throw new wire_LocatorPropagationException();
    }
    $this->call_stack++;
    try {
  ...
  function createArgs($classname, $args) {
    ...
    $this->call_stack++;
    try {
      if (!$args instanceOf wire_UserlandArguments) {
        $args = new wire_UserlandArguments($args);
      }
      $factory = $this->findFactory($classname);
   ...
   * @param  $factory    wire_iClassFactory  The factory to use for cvreating instances of this class.
   */
  function registerFactory($classname, wire_iClassFactory $factory) {
  ...
  function register($classname) {
    return $this->registerFactory($classname, new wire_ConfigurableFactory($this));
  }
  ...
  function registerConstructor($classname, $callback) {
    ...
    return $this->registerFactory($classname, new wire_CallbackFactory($this, $callback));
  }
